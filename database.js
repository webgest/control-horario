...
db.exec(`
  ...
  CREATE INDEX IF NOT EXISTS idx_fichajes_trabajadora_fecha
    ON fichajes(trabajadora_id, fecha);

  CREATE INDEX IF NOT EXISTS idx_trabajadoras_dni
    ON trabajadoras(dni);
`);

// ─── FUNCIÓN AUXILIAR: PIN por defecto (últimos 4 dígitos del DNI/NIE) ─────────
...
function seedData() {
  ...
  if (countEmpresas > 0) return; // Ya inicializado

  console.log('🌱 Inicializando datos iniciales...');

  // ── Admin por defecto ──
  const adminPass = bcrypt.hashSync(
    process.env.ADMIN_PASSWORD || 'Webgest2026!',
    10
  );
  db.prepare(`
    INSERT OR IGNORE INTO admin_users (username, password, nombre)
    VALUES (?, ?, ?)
  `).run(process.env.ADMIN_USERNAME || 'webgest', adminPass, 'Administrador Webgest');

  // ── Helper: insertar empresa ──
  const insertEmpresa = db.prepare(`
    INSERT INTO empresas (nombre, nif, ccc) VALUES (?, ?, ?)
  `);

  // ── Helper: insertar trabajadora ──
  const insertTrabajadora = db.prepare(`
    INSERT INTO trabajadoras
      (empresa_id, nombre, dni, pin, telefono, horas_dia, dias_mes,
       tipo_jornada, estado, observaciones, horas_pendientes_confirmar)
    VALUES
      (@empresa_id, @nombre, @dni, @pin, @telefono, @horas_dia, @dias_mes,
       @tipo_jornada, @estado, @observaciones, @pendiente)
  `);

  function addWorker(empresaId, nombre, dni, horasDia, diasMes, tipoJornada,
                     estado, observaciones, pendiente) {
    const pin = bcrypt.hashSync(defaultPin(dni), 10);
  ...
  const H = HORAS_COMPLETA; // 8.30h jornada completa

  // ────────────────────────────────────────────────────────
  // EMPRESA 1 — CONTRATACIONES LIMPIMUR, S.L.
  // ────────────────────────────────────────────────────────
  const e1 = insertEmpresa.run('CONTRATACIONES LIMPIMUR, S.L.', 'B30381230', '30/1009098-56').lastInsertRowid;

  // ILT: trabajadora en baja médica — horas establecidas por defecto, admin debe revisar
  addWorker(e1, 'Miñarro García, Isabel Pilar',    '23233727R', H,    30, 'parcial',  'IT',
    'ILT - En incapacidad temporal. Horas según subsidio pendientes de confirmar.', true);

  // Jornada completa — 31 días
  addWorker(e1, 'Monteagudo Pujalte, María Carmen', '27476364N', H,    31, 'completa', 'activa', null, false);

  // Parciales con base salarial — horas estimadas, pendientes de confirmar por Webgest
  // Base 734,70€ vs referencia ~1.260€ → ~4,84h/día (PENDIENTE CONFIRMAR)
  addWorker(e1, 'Navarro Rodríguez, Francisca',    '23226904D', 4.84, 30, 'parcial',  'activa',
    'Parcial base 734,70€. Horas estimadas (pendiente confirmar en nómina).', true);

  // Base 639,17€ → ~4,21h/día (PENDIENTE CONFIRMAR)
  addWorker(e1, 'Palomeque Pérez, Vanessa Mabel',  '60246144J', 4.21, 30, 'parcial',  'activa',
    'Parcial base 639,17€. Horas estimadas (pendiente confirmar en nómina).', true);

  // Base 1.103,71€ → ~7,27h/día (PENDIENTE CONFIRMAR)
  addWorker(e1, 'Sánchez Jiménez, Juana María',    '23280620C', 7.27, 30, 'parcial',  'activa',
    'Parcial base 1.103,71€. Horas estimadas (pendiente confirmar en nómina).', true);

  // ────────────────────────────────────────────────────────
  // EMPRESA 2 — GRUPO LIMPIMUR EXPANSIÓN, S.L.
  // ────────────────────────────────────────────────────────
  ...
  // Base 752,13€ → ~4,95h/día (PENDIENTE CONFIRMAR)
  addWorker(e2, 'Vargas Aguilera, José',       '23249444D', 4.95, 30, 'parcial',  'activa',
    'Parcial base 752,13€. Horas estimadas (pendiente confirmar en nómina).', true);

  // ────────────────────────────────────────────────────────
  // EMPRESA 3 — CAMPICO BLANCO SCOOP
  // ────────────────────────────────────────────────────────
  ...
  addWorker(e3, 'González Venteo, María José',       '17469048L', 4.00, 20, 'parcial',  'activa',
    'Alta 12/05/2026.', false);
  ...
  addWorker(e3, 'Quiñonero Pérez, María Luz',        '23293489D', 5.00, 20, 'parcial',  'activa',
    'IT parcial mayo 2026.', false);
  ...
  addWorker(e4, 'Reinaldos López, Ascensión',        '23227468K', H,    30, 'completa', 'IT',
    'IT completa mayo 2026.', false);
  addWorker(e4, 'Salinas Macas, Lady Tamara',        '30296461X', 5.99, 21, 'parcial',  'activa', null, false);
  addWorker(e4, 'Vera Ruiz, María Isabel',           '23252196R', 3.99,  4, 'parcial',  'baja',
    'Baja desde 04/05/2026.', false);

  // ────────────────────────────────────────────────────────
  // EMPRESA 5 — SGN AYUDA A DOMICILIO S. COOP
  // ────────────────────────────────────────────────────────
...
const queries = {
  // Auth
  getWorkerByDni: db.prepare(`
    SELECT t.*, e.nombre AS empresa_nombre
    FROM trabajadoras t
    JOIN empresas e ON t.empresa_id = e.id
    WHERE t.dni = ? AND t.activa = 1
  `),
  ...
  // Fichajes
  getFichajeHoyByTrabajadora: db.prepare(`
    SELECT * FROM fichajes
    WHERE trabajadora_id = ? AND fecha = ?
    ORDER BY id DESC LIMIT 1
  `),
  getFichajeAbierto: db.prepare(`
    SELECT f.*, t.nombre AS trabajadora_nombre, t.horas_dia, t.telefono,
           e.nombre AS empresa_nombre
    FROM fichajes f
    JOIN trabajadoras t ON f.trabajadora_id = t.id
    JOIN empresas e ON t.empresa_id = e.id
    WHERE f.trabajadora_id = ? AND f.hora_salida IS NULL
    ORDER BY f.id DESC LIMIT 1
  `),
  createFichaje: db.prepare(`
    INSERT INTO fichajes (trabajadora_id, fecha, hora_entrada)
    VALUES (?, ?, ?)
  `),
  closeFichaje: db.prepare(`
    UPDATE fichajes
    SET hora_salida = ?,
        horas_trabajadas = ?,
  ...
  getHistorialMes: db.prepare(`
    SELECT f.*,
           round((julianday(COALESCE(f.hora_salida, ?))
                  - julianday(f.hora_entrada)) * 24, 2) AS horas_calc
    FROM fichajes f
    WHERE f.trabajadora_id = ?
      AND strftime('%Y-%m', f.fecha) = ?
    ORDER BY f.fecha DESC
  `),
  getHistorialRango: db.prepare(`
    SELECT f.*, t.nombre AS trabajadora_nombre, t.dni,
           e.nombre AS empresa_nombre, e.id AS empresa_id
    FROM fichajes f
    JOIN trabajadoras t ON f.trabajadora_id = t.id
    JOIN empresas e ON t.empresa_id = e.id
    WHERE (@empresa_id IS NULL OR e.id = @empresa_id)
      AND (@trabajadora_id IS NULL OR f.trabajadora_id = @trabajadora_id)
      AND (@fecha_inicio IS NULL OR f.fecha >= @fecha_inicio)
      AND (@fecha_fin IS NULL OR f.fecha <= @fecha_fin)
    ORDER BY e.nombre, t.nombre, f.fecha DESC
    LIMIT 500
  `),

  // Admin — Trabajadoras
  getAllTrabajadoras: db.prepare(`
  ...
  `),
  getTrabajadorasByEmpresa: db.prepare(`
    SELECT * FROM trabajadoras WHERE empresa_id = ? AND activa = 1 ORDER BY nombre
  `),
  getTrabajadoraById: db.prepare(`
    SELECT t.*, e.nombre AS empresa_nombre FROM trabajadoras t
    JOIN empresas e ON t.empresa_id = e.id WHERE t.id = ?
  `),
  updateTrabajadora: db.prepare(`
    UPDATE trabajadoras
    SET nombre = @nombre, telefono = @telefono, horas_dia = @horas_dia,
        dias_mes = @dias_mes, tipo_jornada = @tipo_jornada,
        estado = @estado, observaciones = @observaciones,
        horas_pendientes_confirmar = @pendiente
    WHERE id = @id
  `),
  insertTrabajadora: db.prepare(`
    INSERT INTO trabajadoras
      (empresa_id, nombre, dni, pin, telefono, horas_dia, dias_mes,
       tipo_jornada, estado, observaciones)
    VALUES (@empresa_id, @nombre, @dni, @pin, @telefono, @horas_dia,
            @dias_mes, @tipo_jornada, @estado, @observaciones)
  `),
  deactivateTrabajadora: db.prepare(`
    UPDATE trabajadoras SET activa = 0, estado = 'baja' WHERE id = ?
  `),
  updatePin: db.prepare(`
    UPDATE trabajadoras SET pin = ? WHERE id = ?
  `),
  ...
  // Admin — Dashboard
  getEstadoHoyByEmpresa: db.prepare(`
    SELECT
      t.id, t.nombre, t.dni, t.estado, t.horas_dia, t.tipo_jornada,
      t.horas_pendientes_confirmar,
      f.id AS fichaje_id,
      f.hora_entrada,
      f.hora_salida,
      f.horas_trabajadas,
      f.observaciones AS fichaje_obs
    FROM trabajadoras t
    LEFT JOIN fichajes f
      ON f.trabajadora_id = t.id
      AND f.fecha = ?
    WHERE t.empresa_id = ? AND t.activa = 1
    ORDER BY t.nombre
  `),

  // Admin — Fichajes open (para cron)
  getFichajesAbiertos: db.prepare(`
    SELECT f.id, f.trabajadora_id, f.hora_entrada,
           t.horas_dia, t.nombre AS trabajadora_nombre, t.telefono,
           e.nombre AS empresa_nombre
    FROM fichajes f
    JOIN trabajadoras t ON f.trabajadora_id = t.id
    JOIN empresas e ON t.empresa_id = e.id
  ...
  // Admin — Modificar fichaje (con auditoría)
  updateFichaje: db.prepare(`
    UPDATE fichajes
    SET hora_entrada = @hora_entrada, hora_salida = @hora_salida,
        horas_trabajadas = @horas_trabajadas, observaciones = @observaciones,
        modificado_por = @admin, modificado_en = @modificado_en,
  ...
  insertAudit: db.prepare(`
    INSERT INTO audit_log (admin_username, accion, tabla, registro_id,
                           datos_anteriores, datos_nuevos)
    VALUES (?, ?, ?, ?, ?, ?)
  `),

  // Informe mensual para PDF
  getInformeMensual: db.prepare(`
    SELECT f.fecha, f.hora_entrada, f.hora_salida, f.horas_trabajadas,
           f.cerrado_automaticamente, f.observaciones,
           f.modificado_por, f.razon_modificacion,
           t.nombre AS trabajadora_nombre, t.dni, t.tipo_jornada, t.horas_dia,
           e.nombre AS empresa_nombre, e.nif AS empresa_nif, e.ccc
    FROM fichajes f
    JOIN trabajadoras t ON f.trabajadora_id = t.id
    JOIN empresas e ON t.empresa_id = e.id
    WHERE f.trabajadora_id = ?
...
/**
 * Convierte horas decimales a string HH:MM
 * 8.30 → "8:18", 5.99 → "5:59"
 */
...
/**
 * Calcula la hora de fin de jornada dado un inicio (string ISO) y horas_dia
 * Devuelve un objeto Date
 */
function calcularHoraFin(horaEntrada, horasDia) {
  const entrada = new Date(horaEntrada);
  const segundos = Math.round(horasDia * 3600);
  return new Date(entrada.getTime() + segundos * 1000);
}

/**
 * Calcula horas trabajadas entre dos datetimes (decimal)
 */
...
/**
 * Extrae la hora HH:MM de un string datetime ISO/SQLite
 * "2026-06-11 08:30:00" → "08:30"
 */
