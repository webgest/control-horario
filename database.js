...
const queries = {
  ...
  getFichajeHoyByTrabajadora: db.prepare("SELECT * FROM fichajes WHERE trabajadora_id = ? AND fecha = ? ORDER BY id DESC LIMIT 1"),
  ...
  createFichaje: db.prepare("INSERT INTO fichajes (trabajadora_id, fecha, hora_entrada) VALUES (?, ?, ?)"),
  closeFichaje: db.prepare('UPDATE fichajes SET hora_salida = ?, horas_trabajadas = ?, cerrado_automaticamente = ? WHERE id = ?'),
  getHistorialMes: db.prepare("SELECT f.*, round((julianday(COALESCE(f.hora_salida, ?)) - julianday(f.hora_entrada)) * 24, 2) AS horas_calc FROM fichajes f WHERE f.trabajadora_id = ? AND strftime('%Y-%m', f.fecha) = ? ORDER BY f.fecha DESC"),
  ...
  updateFichaje: db.prepare("UPDATE fichajes SET hora_entrada = @hora_entrada, hora_salida = @hora_salida, horas_trabajadas = @horas_trabajadas, observaciones = @observaciones, modificado_por = @admin, modificado_en = @modificado_en, razon_modificacion = @razon WHERE id = @id"),
  ...
  insertPausa:     db.prepare("INSERT INTO pausas (fichaje_id, inicio) VALUES (?, ?)"),
  cerrarPausa:     db.prepare("UPDATE pausas SET fin = ? WHERE fichaje_id = ? AND fin IS NULL"),
  getTotalPausasH: db.prepare("SELECT COALESCE(SUM((julianday(COALESCE(fin,?))-julianday(inicio))*24),0) AS total FROM pausas WHERE fichaje_id = ?"),
...
function calcularHoraFin(horaEntrada, horasDia) {
  // Añadir 'Z' para que Node.js interprete el string UTC almacenado como UTC, no como hora local
  const entrada = new Date(String(horaEntrada).replace(' ', 'T') + 'Z');
...
function horaDisplay(isoStr) {
  if (!isoStr) return '—';
  try {
    const d = new Date(String(isoStr).replace(' ', 'T') + 'Z');
    if (isNaN(d)) return String(isoStr).slice(11, 16);
    // Calcular offset España (+1 CET, +2 CEST) — aritmética pura UTC
